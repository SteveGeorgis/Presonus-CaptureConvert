# Last update:  v1.3.0 on 2025-04-27 by SG
...
### v1.3.0 4-27-25 bug fix: added the ability to include channels as ch33 and ch34 (the Main channels which are stored in a separate section of .scn)
...
# Also get the mixer track names from the .scn file and update the .capture track names
    ...
    for fname in cap_list:      
        ...
        # get and update the metadata from scene[][]
        track_num = 0
        for i in scn:
            scene[i] = scn[i]
            ...
            if i == 'line': 
                for k in scene[i]:
                    track_num +=1
                    line_info[k] = {'color': scene[i][k]['color'], 'username': scene[i][k]['username'], 'link' : scene[i][k]['link'], 'trk_num' : " " }
                    ...
                    if scene[i][k]['color'] > 2**31:
                        scene[i][k]['color'] -= 2**32
                    if scene[i][k]['aux_asn_flags']:
                        scene[i][k]['aux_asn_flags'] = -1
                    if scene[i][k]['fx_asn_flags']:
                        scene[i][k]['fx_asn_flags'] = 255

            ### create ch 33 and 34 entries to be able to capture the Main channels ####

            if i == 'main':
                line_info['ch33'] = {'color': 0, 'username': "Main" , 'link': 1, 'linkmaster': 1, 'trk_num': "33", 'aux_asn_flags': -1, 'fx_asn_flags': 255}
                line_info['ch34'] = {'color': 0, 'username': "Main",  'link': 1, 'linkmaster': 0, 'trk_num': "34", 'aux_asn_flags': -1, 'fx_asn_flags': 255}

                # --- Update scene['line'] dictionary too ---
                if 'line' not in scene:
                    scene['line'] = {}
                scene['line']['ch33'] = line_info['ch33']
                scene['line']['ch34'] = line_info['ch34']
        ...
        for i in capture.findall('AudioTrack'):
            ...
            # add speaker flag based on the link flag.  this is used by StudioOne to make the track stereo or mono.  1=mono, 3=stereo
